...
def blockstates_json(base_block_name, blockstates_dir, i, letters, slab_block_name):
    types = ["bottom", "top", "double"]
    ...
    for type in types:
        model = f"cheatmusicblockmod:block/cheat_music_block_{base_block_name}"
        if type == "bottom":
            model = f"cheatmusicblockmod:block/cheat_music_block_{slab_block_name}"
        if type == "top":
            model = f"cheatmusicblockmod:block/cheat_music_block_{slab_block_name}_top"#todo top?

        for j in range(len(letters)):
            letter = letters[j]
            blockstates_string = blockstates_string + f'"type={type},type2={j}":  {{ "model": "{model}_{letter}"}},\n'
    ...
    with open(os.path.join(blockstates_dir, f"cheat_music_block_{slab_block_name}{i}.json"), "w", encoding="utf-8") as file:
...
def block_json(base_block_name, letter_and_accidental, block_dir, slab_block_name):
    block_string = f"""
{{
...
"""
    with open(os.path.join(block_dir, f"cheat_music_block_{slab_block_name}_{letter_and_accidental}.json"), "w", encoding="utf-8") as file:
        file.write(block_string)

def block_json_top(base_block_name, letter_and_accidental, block_dir, slab_block_name):
    block_string = f"""
{{
...
"""
    with open(os.path.join(block_dir, f"cheat_music_block_{slab_block_name}_top_{letter_and_accidental}.json"), "w", encoding="utf-8") as file:
        file.write(block_string)
...
def main():
    ...
    for t in types:
        ...
        block_json_top(args.base_block_name, t, block_dir, args.name)
    for i in range(7):
        letter = types[i]
