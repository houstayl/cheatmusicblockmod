...
public class CheatMusicSlabsBlock extends SlabBlock {

    public static final IntProperty TYPE = IntProperty.of("type2", 0, 34);
    ...
    public CheatMusicSlabsBlock(Settings settings) {
        super(settings.luminance(state -> 15));
        setDefaultState(getStateManager().getDefaultState().with(TYPE, 0));
    }

    public CheatMusicSlabsBlock(Settings settings, int type) {
        super(settings.luminance(state -> 15));
    ...
    public void onPlaced(World world, BlockPos pos, BlockState state, @Nullable LivingEntity placer, ItemStack itemStack) {
        ...
        CheatMusicBlockMod.cheatMusicBlockCoordinates[state.get(CheatMusicSlabsBlock.TYPE)].add(pos.toImmutable());
    }
    ...
    private void nextType(BlockState state, World world, BlockPos pos, PlayerEntity player) {
        int newType = (state.get(CheatMusicSlabsBlock.TYPE) + 1) % 35;
